...
class UserController extends Controller
{
    ...
    return DataTables::of($users)
        ...
        ->addColumn('aksi', function ($user) {
            ...
            $btn = '<a href="' . url('/user/' . $user->level_id) . '" class="btn btn-info btn-sm">Detail</a> ';
            $btn .= '<button onclick="modalAction(\''.url('/user/' . $user->level_id . '/edit_ajax').'\')" class="btn btn-warning btn-sm">Edit</button>';
            $btn .= '<button onclick="modalAction(\''.url('/user/' . $user->level_id . '/delete_ajax').'\')" class="btn btn-danger btn-sm">Hapus</button> ';
            return $btn;
        })
    ...
    public function update(Request $request, string $id)
    {
        // Validasi input
        $request->validate([
            'nip'      => 'required|string|min:3|unique:users,nip,' . $id, // Validasi untuk kolom nip
            'nama'     => 'required|string|max:100',
            'email'    => 'required|email|unique:users,email,' . $id, // Validasi untuk kolom email
